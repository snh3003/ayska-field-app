...
import { localDataService, type Attendance } from '../../services/LocalDataService';
...
export default function EmployeeHome() {
  ...
  // Stats calculation
  const stats = user.id ? localDataService.getEmployeeStats(user.id) : { totalDays: 0, completedDays: 0, totalVisits: 0, completedVisits: 0 };
  
  // Check if day is started
  React.useEffect(() => {
    if (user.id) {
      const todayAttendance = localDataService.getAll<Attendance>('attendance').find(
        (a: Attendance) => a.employeeId === user.id && a.date === new Date().toISOString().split('T')[0]
      );
  ...
  const endDay = () => {
    if (user.id) {
      ...
      const attendance = localDataService.getAll<Attendance>('attendance').find(
        (a: Attendance) => a.employeeId === user.id && a.date === today && a.status === 'active'
      );
      if (attendance) {
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      >
        ...
        <View style={styles.statsRow}>
          ...
          />
          <StatsCard 
            icon={<Ionicons name="checkmark-done-outline" size={24} color={theme.success} />}
            title="Completed" 
        ...
        <View style={styles.statsRow}>
          <StatsCard 
            icon={<Ionicons name="people-outline" size={24} color={theme.secondary} />}
            title="Total Visits" 
        ...
        <Card variant="elevated">
          <View style={styles.dayControl}>
            <View style={styles.dayControlText}>
              <Text style={[Typography.h4, { color: theme.text, marginBottom: Spacing.xs }]}>
            ...
            <TouchableOpacity
              ...
              style={[
                styles.dayButton,
                { backgroundColor: isDayStarted ? theme.error + '15' : theme.success + '15' }
              ]}
            >
              <Ionicons 
        ...
        <View style={styles.section}>
          ...
          {loading ? (
            ...
            <EmptyState
              ...
              message={searchQuery ? 'Try a different search term' : 'You have no doctors assigned yet'}
            />
          ) : (
          ...
          <Card variant="outlined">
            <View style={styles.activityItem}>
              ...
              <View style={styles.activityContent}>
                <Text style={[Typography.body, { color: theme.text }]}>Day started</Text>
